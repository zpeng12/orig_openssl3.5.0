...
static int add_provider_groups(const OSSL_PARAM params[], void *data)
{
    ...
    SSL_CTX *ctx = pgd->ctx;
    struct ossl_provider_st *provider = pgd->provider;
    const OSSL_PARAM *p;
...
static int discover_provider_groups(struct ossl_provider_st *provider, void *vctx)
{
...
int SSL_CTX_set_tlsext_max_fragment_length(SSL_CTX *ctx, unsigned char mode)
{
...
int SSL_set_tlsext_max_fragment_length(SSL *ssl, unsigned char mode)
{
    if (mode != TLSEXT_max_fragment_length_DISABLED
            && !IS_MAX_FRAGMENT_LENGTH_EXT_VALID(mode)) {
        ERR_raise(ERR_LIB_SSL, SSL_R_SSL3_EXT_INVALID_MAX_FRAGMENT_LENGTH);

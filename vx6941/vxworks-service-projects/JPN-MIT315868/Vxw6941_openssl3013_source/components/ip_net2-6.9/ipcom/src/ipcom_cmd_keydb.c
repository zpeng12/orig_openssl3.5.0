 ...
 *
 *   Document no: @(#) $Name: VXWORKS_ITER29_2014062011 $ $RCSfile: ipcom_cmd_keydb.c,v $ $Revision: 1.6.10.3 $
...
ipcom_cmd_keydb_import(const char* type, const char* key_id, const char* extra)
{
    ...
    if (ipcom_strcmp(type, "buf") == 0)
    {
        ...
        rc = ipcom_key_db_buf_import(key_id, (Ip_u8*)prompted_key[0], (int)ipcom_strlen(prompted_key[0]));
        if (rc != IPCOM_SUCCESS)
            ipcom_printf("Failed: %s"IP_LF, ipcom_err_string(rc));
        return rc;
    }
...
#endif
        ipcom_printf("DSA Public Key: (%d bit)\n",
                     BN_num_bits(pkey->pkey.dsa->params.p));
...
ipcom_cmd_keydb_verify(const char* key_id)
{
    ...
    if ((int)ipcom_strlen(prompted_key) == key_len &&
        ipcom_memcmp(stored_key, prompted_key, (Ip_size_t)key_len) == 0)
    {

...
static void tear_down_keymgmt_internal_test(FIXTURE *fixture)
{
    if (fixture != NULL) {
...
static FIXTURE *set_up_keymgmt_internal_test(const char *testcase_name)
{
    FIXTURE *fixture;
    ...
    if (!TEST_ptr(fixture = OPENSSL_zalloc(sizeof(*fixture)))
        ...
        || !TEST_ptr(fixture->prov2 = OSSL_PROVIDER_load(fixture->ctx2,
                                                         "default"))) {
        tear_down_keymgmt_internal_test(fixture);
        return NULL;
    }
    return fixture;
}
...
{
    SETUP_TEST_FIXTURE(FIXTURE, set_up_keymgmt_internal_test);
    EXECUTE_TEST(tests[n], tear_down_keymgmt_internal_test);
    return result;
}
...
int test_internal_keymgmt(void)
{
#if 0
    if (!TEST_ptr(cert_filename = test_get_argument(0)))	/*"test", "certs", "ee-cert.pem"*/
        return 0;
#endif
	cert_filename = "ee-cert.pem";

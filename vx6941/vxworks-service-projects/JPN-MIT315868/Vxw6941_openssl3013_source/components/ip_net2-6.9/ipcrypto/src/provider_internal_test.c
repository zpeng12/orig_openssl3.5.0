...
#if 0
    ret =
        TEST_true(ossl_provider_activate(prov, 1, 0))			//1st error here, ???
...
TEST_ptr(prov = ossl_provider_find(NULL, name, 0));			/*not find*//*root error: */
...
int test_internal_provider(int i)
{
if( (i == 1 )  || ( i == 0 ) )
{

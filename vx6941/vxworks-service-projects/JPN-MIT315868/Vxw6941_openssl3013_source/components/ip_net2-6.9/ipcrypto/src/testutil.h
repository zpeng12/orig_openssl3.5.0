...
# define ADD_TEST(test_function) \
	if(SSL_TEST_FAIL != test_function())				\
		printf("PASS		%s.\r\n", #test_function);			\
	else									\
		printf("FAIL	%s.\r\n", #test_function);	

# define ADD_ALL_TESTS(test_function, num) \
	if(SSL_TEST_FAIL != test_function(num))				\
		printf("PASS		%s.\r\n", #test_function);			\
	else									\
		printf("FAIL	%s.\r\n", #test_function);
 ...
 * context.
 */
int test_get_libctx(OSSL_LIB_CTX **libctx, struct ossl_provider_st **default_null_prov,
                    const char *config_file,
                    struct ossl_provider_st **provider, const char *module_name);
int test_arg_libctx(OSSL_LIB_CTX **libctx, struct ossl_provider_st **default_null_prov,
                    struct ossl_provider_st **provider, int argn, const char *usage);
...
#else
# define TEST_mem_eq(a, m, b, n) test_mem_eq(__FUNCTION__, __LINE__, #a, #b, a, m, b, n)
# define TEST_mem_ne(a, m, b, n) test_mem_ne(__FUNCTION__, __LINE__, #a, #b, a, m, b, n)
#endif
...
struct ossl_provider_st *fake_rand_start(OSSL_LIB_CTX *libctx);
void fake_rand_finish(struct ossl_provider_st *p);

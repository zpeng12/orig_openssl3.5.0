...
int test_bn_output(int n)
{
    BIGNUM *b = NULL;

    if(n > OSSL_NELEM(bn_output_tests) - 1)
		test_printf_stderr("........[%s, %d] 	n out of scope.\r\n", __FUNCTION__, __LINE__);
...
int test_test(void)
{
    ...
    ADD_ALL_TESTS(test_bn_output, OSSL_NELEM(bn_output_tests) - 1 );

...
void test_open_streams(void)
{
    ...
    bio_err_test = BIO_push(BIO_new(BIO_f_prefix()), tap_err);
    BIO_set_prefix(bio_out, "# ");
    BIO_set_prefix(bio_err_test, "# ");

    OPENSSL_assert(bio_out != NULL);
    OPENSSL_assert(bio_err_test != NULL);
}
...
{
    return BIO_vprintf(bio_err_test, fmt, ap);
}

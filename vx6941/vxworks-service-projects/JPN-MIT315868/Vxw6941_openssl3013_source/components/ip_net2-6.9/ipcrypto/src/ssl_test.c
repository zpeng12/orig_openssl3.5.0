...
test_ctx->method = SSL_TEST_METHOD_TLS;
...
#define SSL_TEST_USAGE "conf_file module_name [module_conf_file]\n"
#if 0	
OPT_TEST_DECLARE_USAGE(USAGE)

#endif

int test_ssl_new(void)
{
    long num_tests;
...
#endif 
            || !TEST_int_gt(NCONF_load(conf, "01-simple.txt", NULL), 0)
            || !TEST_int_ne(NCONF_get_number_e(conf, NULL, "num_tests",
                                               &num_tests), 0)) {
        TEST_error("usage: ssl_test %s", SSL_TEST_USAGE);
        return 0;
    }

    if (!test_arg_libctx(&libctx, &defctxnull, &thisprov, 1, SSL_TEST_USAGE))
        return 0;

	load_ca(1);
    ADD_ALL_TESTS(test_handshake, (int)num_tests - 1);
    return 1;
}

void ssl_all_test_cleanup_tests(void)
{
    NCONF_free(conf);

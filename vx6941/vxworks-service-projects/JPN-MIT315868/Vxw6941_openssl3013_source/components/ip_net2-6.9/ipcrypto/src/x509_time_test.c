...
static int test_x509_time_local(int idx)
{
    ASN1_TIME *t = NULL;
    int result, rv = 0;
    ...
    if (x509_format_tests[idx].set_string) {
        ...
        t = ASN1_TIME_new();
        if (t == NULL) {
            TEST_info("test_x509_time_local(%d) failed: internal error\n", idx);
            return 0;
        }
    }
    ...
    if (!TEST_int_eq(result, x509_format_tests[idx].expected)) {
        TEST_info("test_x509_time_local(%d) failed: expected %d, got %d\n",
    ...
    if (t != NULL && x509_format_tests[idx].expected_type != -1) {
        if (!TEST_int_eq(t->type, x509_format_tests[idx].expected_type)) {
            TEST_info("test_x509_time_local(%d) failed: expected_type %d, got %d\n",
    ...
    if (t != NULL && x509_format_tests[idx].expected_string) {
        if (!TEST_mem_eq((const char *)t->data, t->length,
            ...
            TEST_info("test_x509_time_local(%d) failed: expected_string %s, got %.*s\n",
...
int test_x509_time(void)
{
    ADD_TEST(test_x509_cmp_time_current);
    ADD_TEST(test_X509_cmp_timeframe);
    ADD_ALL_TESTS(test_x509_cmp_time, OSSL_NELEM(x509_cmp_tests));
    ADD_ALL_TESTS(test_x509_time_local, OSSL_NELEM(x509_format_tests));

...
static OSSL_LIB_CTX *get_thread_default_context(void)
{
    if (!RUN_ONCE(&default_context_init, default_context_do_init))
    	{
		printf("get_thread_default_context: NULL.\r\n");
        	return NULL;
	}
...
struct ossl_lib_ctx_st *ossl_lib_ctx_get_concrete(struct ossl_lib_ctx_st *ctx)
{
#ifndef FIPS_MODULE
    if (ctx == NULL)
...
#if 1		
	test_printf_stderr("........[%s, %d] static_index = %d.\r\n", __FUNCTION__, __LINE__, static_index);
#endif
                            ...
                            const OSSL_LIB_CTX_METHOD *meth)
{
    ...
    {
		ctx->lock = CRYPTO_THREAD_lock_new();
		ctx->index_locks[index] = CRYPTO_THREAD_lock_new();
	}
	...
	test_printf_stderr("........[%s, %d] ctx init with meth ok.\r\n", __FUNCTION__, __LINE__);
#endif
...
#if 1				
		test_printf_stderr("........[%s, %d] ctx get data ok.\r\n", __FUNCTION__, __LINE__);
#endif
        CRYPTO_THREAD_unlock(ctx->lock);
    }

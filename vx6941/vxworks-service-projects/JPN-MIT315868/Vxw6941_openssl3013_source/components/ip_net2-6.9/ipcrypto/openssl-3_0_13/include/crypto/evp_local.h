...
void *evp_generic_fetch_by_number(OSSL_LIB_CTX *ctx, int operation_id,
                                  ...
                                  int (*up_ref_method)(void *),
                                  void (*free_method)(void *));
void *evp_generic_fetch_from_prov(struct ossl_provider_st *prov, int operation_id,
                                  const char *name, const char *properties,
                                  void *(*new_method)(int name_id,
                                                      const OSSL_ALGORITHM *algodef,
                                                      struct ossl_provider_st *prov),
...
EVP_SIGNATURE *evp_signature_fetch_from_prov(struct ossl_provider_st *prov,
                                             const char *name,
                                             const char *properties);
EVP_ASYM_CIPHER *evp_asym_cipher_fetch_from_prov(struct ossl_provider_st *prov,
                                                 const char *name,
                                                 const char *properties);
EVP_KEYEXCH *evp_keyexch_fetch_from_prov(struct ossl_provider_st *prov,
                                         const char *name,
                                         const char *properties);
EVP_KEM *evp_kem_fetch_from_prov(struct ossl_provider_st *prov,
                                 const char *name,
                                 const char *properties);
...
int evp_is_a(struct ossl_provider_st *prov, int number,
             const char *legacy_name, const char *name);
int evp_names_do_all(struct ossl_provider_st *prov, int number,

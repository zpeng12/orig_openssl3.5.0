...
#if 1
/*if defined(OPENSSL_SYSTEM_VXWORKS)*/
...
size_t ossl_pool_acquire_entropy(RAND_POOL *pool)
{
	...
	buffer.pid = ipcom_getpid();

	while(pool->len < pool->min_len)
	{
#ifdef INCLUDE_TIMESTAMP
		buffer.timestamp = sysTimestamp();
#endif
	    ossl_rand_pool_add(pool, (unsigned char *)&buffer, sizeof(buffer), 8 * sizeof(buffer));
	}
    /*

...
int OSSL_PARAM_get_int32(const OSSL_PARAM *p, int *val)
{
    if (val == NULL || p == NULL )
        return 0;
...
int OSSL_PARAM_set_int32(OSSL_PARAM *p, int val)
{
    if (p == NULL)
        return 0;
    p->return_size = 0;
...
OSSL_PARAM OSSL_PARAM_construct_int32(const char *key, int *buf)
{
    return ossl_param_construct(key, OSSL_PARAM_INTEGER, buf,
                                sizeof(int32_t));
}

int OSSL_PARAM_get_uint32(const OSSL_PARAM *p, unsigned int *val)
{
    if (val == NULL || p == NULL)
        return 0;
...
int OSSL_PARAM_set_uint32(OSSL_PARAM *p, unsigned int val)
{
    if (p == NULL)
        return 0;
    p->return_size = 0;
...
OSSL_PARAM OSSL_PARAM_construct_uint32(const char *key, unsigned int *buf)
{
    return ossl_param_construct(key, OSSL_PARAM_UNSIGNED_INTEGER, buf,
                                sizeof(uint32_t));
}

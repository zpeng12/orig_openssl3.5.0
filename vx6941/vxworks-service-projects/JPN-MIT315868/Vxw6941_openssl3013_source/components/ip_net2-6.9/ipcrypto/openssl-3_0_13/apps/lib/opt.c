...
const char *opt_path_end(const char *filename)
{
    ...
    for (p = filename + ipcom_strlen(filename); --p > filename;)
        if (*p == '/') {
            p++;
            break;
        }
...
void opt_set_fortest(void)
{
	argv[0] = "01-simple.txt";
	argv[1] = "default";
	argv[2] = NULL;
}

void opt_set_fortest_ctx(void)
{
	argv[0] = "ssl_test_ctx_test_cnf.txt";
	argv[1] = NULL;
}

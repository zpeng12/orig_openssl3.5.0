...
#include "crypto/dsa_local.h"
#include "crypto/rsa_local.h"
...
#ifndef MAXPATHLEN
#define MAXPATHLEN 256 /* max length of path */
#endif
...
int	keygen_debug = 0;		/* keygen_debug, not de bug */
...
#ifdef SYS_VXWORKS
extern int ipcom_gettimeofday(struct timeval *tp, void *nullptr);
int ntp_keygen(char *pathbuf);
#ifdef NTP_VXWORKS_6

static char ntp_keygenparams[MAXFILENAME + MAXPATHLEN + 1];
Ip_pid_t  ntpkeygen_pid = 0;

IP_STATIC void ntpkeygen_daemon(void)
{
   ipcom_proc_init();
   if(ntp_keygen(ntp_keygenparams) < 0)
   	   ipcom_printf("Spawn ntpkeygen daemon failed.\n");
   ntpkeygen_pid = 0;
   ipcom_proc_exit();
}

/* The routeine to start ntpkeygen daemon which will be used by ipntp component. */
IP_PUBLIC Ip_err ntpkeygen (char* pathfilename)
{
   ...
   ipcom_memset(ntp_keygenparams, 0, (MAXFILENAME + MAXPATHLEN + 1));
   if((pathfilename != IP_NULL) && (ipcom_strlen(pathfilename)< (MAXFILENAME + MAXPATHLEN + 1)))
       ipcom_strcpy(ntp_keygenparams,pathfilename);
   
   if (ntpkeygen_pid != 0)
   {
	   ipcom_printf ("ntpkeygen already running\n");
	   return IPCOM_ERR_ALREADY_STARTED;
   }
   
   if(ipcom_proc_acreate("ntpkeygen", (Ipcom_proc_func)ntpkeygen_daemon, &attr, &ntpkeygen_pid))
   {
 ...
 */
int
raed_link(char * link, char * file, int len) {
	return (-1);
}
/*
...
#define iffsw   HAVE_OPT(ID_KEY)
	...
	keygen_debug = DESC(DEBUG_LEVEL).optOccCt;
	...
	while (pkey_host == NULL && rval == 0 && !HAVE_OPT(ID_KEY)) {
		...
		if ((fstr = fopen(filename, "r")) != NULL) {
			pkey_host = PEM_read_PrivateKey(fstr, NULL,
			    NULL, passwd1);
			fclose(fstr);
			raed_link(filename, filename,  sizeof(filename));
			if (pkey_host == NULL) {
...
/*3_0_13 update*/
	...
	if (pkey_iff != NULL && rval == 0 && HAVE_OPT(ID_KEY)) {
        ...
        if(sptr != NULL)
        {
            tld = ipcom_malloc(ipcom_strlen(sptr));	/* we have an extra byte ... */
...
#ifdef NTP_VXWORKS_6
	BN_GENCB_set(&gencb,  ncb, "DSA");

	dsa = DSA_new();
	if(dsa != NULL)
	    DSA_generate_parameters_ex(dsa, modulus, seed, sizeof(seed), NULL, NULL, &gencb );
#else
...
#endif
/*3_0_13 update*/
	BN_rand(b, BN_num_bits(dsa->params.q), -1, 0);	/* a */
	BN_mod(b, b, dsa->params.q, ctx);
	BN_sub(v, dsa->params.q, b);
	BN_mod_exp(v, dsa->params.g, v, dsa->params.p, ctx); /* g^(q - b) mod p */
	BN_mod_exp(u, dsa->params.g, b, dsa->params.p, ctx);	/* g^b mod p */
	BN_mod_mul(u, u, v, dsa->params.p, ctx);
	 ...
	 * modulus q.
	 */
	BN_rand(r, BN_num_bits(dsa->params.q), -1, 0);	/* r */
	BN_mod(r, r, dsa->params.q, ctx);
	 ...
	 */
	BN_rand(k, BN_num_bits(dsa->params.q), -1, 0);	/* k, 0 < k < q  */
	BN_mod(k, k, dsa->params.q, ctx);
	BN_mod_mul(v, dsa->priv_key, r, dsa->params.q, ctx); /* b r mod q */
	BN_add(v, v, k);
	BN_mod(v, v, dsa->params.q, ctx);		/* y = k + b r mod q */
	BN_mod_exp(u, dsa->params.g, k, dsa->params.p, ctx);	/* x = g^k mod p */
	 ...
	 */
	BN_mod_exp(v, dsa->params.g, v, dsa->params.p, ctx); /* g^y mod p */
	BN_mod_exp(w, dsa->pub_key, r, dsa->params.p, ctx); /* v^r */
	BN_mod_mul(v, w, v, dsa->params.p, ctx);	/* product mod p */
	temp = BN_cmp(u, v);
	fprintf(stderr,
	...
	)
{
	...
	dsa->params.q = BN_new();
	dsa->params.g = BN_new();
	s = ipcom_malloc((n + 1) * sizeof(BIGNUM *));
	s1 = ipcom_malloc((n + 1) * sizeof(BIGNUM *));
	for (j = 1; j <= n; j++)
		s1[j] = BN_new();
	temp = 0;
	for (j = 1; j <= n; j++) {
	...
	while (1) {
		...
		BN_one(dsa->params.q);
		for (j = 1; j <= n; j++)
			BN_mul(dsa->params.q, dsa->params.q, s1[j], ctx);
		BN_copy(dsa->params.p, dsa->params.q);
		BN_add(dsa->params.p, dsa->params.p, dsa->params.p);
		BN_add_word(dsa->params.p, 1);
		if (BN_is_prime(dsa->params.p, BN_prime_checks, NULL, ctx,
		    NULL))
			break;
	 ...
	 * q.
	 */
	BN_copy(v, dsa->params.p);
	BN_sub_word(v, 1);
	while (1) {
		BN_rand(dsa->params.g, BN_num_bits(dsa->params.p) - 1, 0, 0);
		BN_mod(dsa->params.g, dsa->params.g, dsa->params.p, ctx);
		BN_gcd(u, dsa->params.g, v, ctx);
		if (!BN_is_one(u))
			continue;

		BN_mod_exp(u, dsa->params.g, dsa->params.q, dsa->params.p, ctx);
		if (BN_is_one(u))
			break;
	}
	...
	fprintf(stderr,
	    ...
	    n, BN_num_bits(dsa->params.q)); 
	x = ipcom_malloc((n + 1) * sizeof(BIGNUM));
	for (j = 1; j <= n; j++) {
		x[j] = BN_new();
		while (1) {
			BN_rand(x[j], BN_num_bits(dsa->params.q), 0, 0);
			BN_mod(x[j], x[j], dsa->params.q, ctx);
			BN_gcd(u, x[j], dsa->params.q, ctx);
			if (BN_is_one(u))
				break;
		}
	}
	 ...
	 */
	a = ipcom_malloc((n + 1) * sizeof(BIGNUM));
	for (i = 0; i <= n; i++) {
		a[i] = BN_new();
		BN_one(a[i]);
	}
	for (j = 1; j <= n; j++) {
		BN_zero(w);
		for (i = 0; i < j; i++) {
			BN_copy(u, dsa->params.q);
			BN_mod_mul(v, a[i], x[j], dsa->params.q, ctx);
			BN_sub(u, u, v);
			BN_add(u, u, w);
			BN_copy(w, a[i]);
			BN_mod(a[i], u, dsa->params.q, ctx);
		}
	}
	...
	g = ipcom_malloc((n + 1) * sizeof(BIGNUM));
	for (i = 0; i <= n; i++) {
		g[i] = BN_new();
		BN_mod_exp(g[i], dsa->params.g, a[i], dsa->params.p, ctx);
	}
	...
	for (j = 1; j <= n; j++) {
		...
		for (i = 0; i <= n; i++) {
			BN_set_word(v, i);
			BN_mod_exp(v, x[j], v, dsa->params.q, ctx);
			BN_mod_mul(v, v, a[i], dsa->params.q, ctx);
			BN_mod_exp(v, dsa->params.g, v, dsa->params.p, ctx);
			BN_mod_mul(u, u, v, dsa->params.p, ctx);
		}
		if (!BN_is_one(u))
			temp = 0;
	...
	for (j = 1; j <= n; j++) {
		for (i = 0; i < n; i++) {
			BN_set_word(v, i);
			BN_mod_exp(v, x[j], v, dsa->params.q, ctx);
			BN_mod_exp(v, g[i], v, dsa->params.p, ctx);
			BN_mod_mul(biga, biga, v, dsa->params.p, ctx);
		}
	}
	 ...
	 */
	while (1) {
		BN_rand(b, BN_num_bits(dsa->params.q), 0, 0);
		BN_mod(b, b, dsa->params.q, ctx);
		BN_gcd(u, b, dsa->params.q, ctx);
		if (BN_is_one(u))
			break;
	}
	BN_mod_inverse(b1, b, dsa->params.q, ctx);
	 ...
	 * key.
	 */
	xbar = ipcom_malloc((n + 1) * sizeof(BIGNUM));
	xhat = ipcom_malloc((n + 1) * sizeof(BIGNUM));
	for (j = 1; j <= n; j++) {
		...
		for (i = 1; i <= n; i++) {
			if (i == j)
				continue;
			BN_mod_exp(u, x[i], v, dsa->params.q, ctx);
			BN_add(xbar[j], xbar[j], u);
		}
		BN_mod_mul(xbar[j], xbar[j], b1, dsa->params.q, ctx);
		BN_mod_exp(xhat[j], x[j], v, dsa->params.q, ctx);
		BN_mod_mul(xhat[j], xhat[j], s[j], dsa->params.q, ctx);
	}
	 ...
	 */
	ss = BN_new();
	BN_copy(ss, dsa->params.q);
	BN_div(ss, u, dsa->params.q, s1[n], ctx);
	...
	BN_mod_exp(bige, biga, ss, dsa->params.p, ctx);
	BN_mod_exp(gbar, dsa->params.g, ss, dsa->params.p, ctx);
	BN_mod_mul(v, ss, b, dsa->params.q, ctx);
	BN_mod_exp(ghat, dsa->params.g, v, dsa->params.p, ctx);
	 ...
	 */
	sdsa = DSA_new();
	sdsa->params.p = BN_dup(dsa->params.p);
	sdsa->params.q = BN_dup(BN_value_one());
	sdsa->params.g = BN_dup(BN_value_one());
	sdsa->priv_key = BN_new();
	sdsa->pub_key = BN_new();
	for (j = 1; j <= n; j++) {
		BN_copy(sdsa->priv_key, xbar[j]);
		BN_copy(sdsa->pub_key, xhat[j]);
		BN_mod_exp(v, dsa->priv_key, sdsa->pub_key, dsa->params.p,
		    ctx);
		BN_mod_exp(u, dsa->pub_key, sdsa->priv_key, dsa->params.p,
		    ctx);
		BN_mod_mul(u, u, v, dsa->params.p, ctx);
		BN_mod_mul(u, u, dsa->params.g, dsa->params.p, ctx);
	...
	)
{
	...
	X509_NAME_add_entry_by_txt(subj, "commonName", MBSTRING_ASC,
	    (unsigned char *) hostname, (int)strlen(hostname), -1, 0);
	subj = X509_get_issuer_name(cert);
	X509_NAME_add_entry_by_txt(subj, "commonName", MBSTRING_ASC,
	    (unsigned char *) trustname, (int)strlen(trustname), -1, 0);

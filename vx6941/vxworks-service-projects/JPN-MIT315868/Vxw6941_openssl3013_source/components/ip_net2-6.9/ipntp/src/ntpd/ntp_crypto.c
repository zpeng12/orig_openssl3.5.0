...
#ifdef OPENSSL_TYPES_H
#undef OPENSSL_TYPES_H
#else
#error "openssl type compatible"
#endif
	...
	)
{
	 ...
	 */
	while (1) {
		keyid = (keyid_t)(ntp_random() + NTP_MAXKEY + 1) & ((1 <<
		    sizeof(keyid_t)) - 1);
	...
	)
{
	...
	while ((rbufp->recv_length - authlen) > MAX_MAC_LEN) {
	...
	)
{
	...
	len = ntohl(ep->vallen);
	ptr = (u_char *)ep->pkt;
	pkey = d2i_PublicKey(EVP_PKEY_RSA, NULL, (const unsigned char **)&ptr, len);
	if (pkey == NULL) {
	...
	)
{
	...
	Ip_size_t	len;		/* extension field length */
...
{
	struct dsa_st	*dsa;		/* IFF parameters */
...
BIGNUM *	test;
	...
	/*3_0_13 upgrade*/
	BN_mod(peer->iffval, peer->iffval, dsa->params.q, bctx);
	BN_CTX_free(bctx);
	...
	)
{
	...
	/*3_0_13 upgrade*/
	BN_mod_mul(bn, dsa->priv_key, r, dsa->params.q, bctx); /* b r mod q */
	BN_add(bn, bn, bk);
	/*BN_mod(bn, bn, dsa->q, bctx);*/		/* k + b r mod q */
	/*3_0_13 upgrade*/
	BN_mod(bn, bn, dsa->params.q, bctx);		/* k + b r mod q */
	
	sdsa->r = BN_dup(bn);
	/*BN_mod_exp(bk, dsa->g, bk, dsa->p, bctx);*/ /* g^k mod p */
	/*3_0_13 upgrade*/
	BN_mod_exp(bk, dsa->params.g, bk, dsa->params.p, bctx); /* g^k mod p */
	bighash(bk, bk);
	sdsa->s = BN_dup(bk);
	BN_CTX_free(bctx);
	...
	)
{
	...
	#endif
	BN_mod_exp(bn, dsa->pub_key, peer->iffval, dsa->params.p, bctx);
	BN_mod_exp(bk, dsa->params.g, sdsa->r, dsa->params.p, bctx);
	BN_mod_mul(bn, bn, bk, dsa->params.p, bctx);
	...
	)
{
	...
	/*3_0_13 update*/
	BN_mod(peer->iffval, peer->iffval, dsa->params.p, bctx);
...
#endif
/*3_0_13 update*/
	sdsa->params.p = BN_new(); sdsa->params.q = BN_new(); sdsa->params.g = BN_new();
	while (1) {
		BN_rand(k, BN_num_bits(dsa->params.q), 0, 0);
		BN_mod(k, k, dsa->params.q, bctx);
		BN_gcd(u, k, dsa->params.q, bctx);
		if (BN_is_one(u))
			break;
	}
	BN_mod_exp(u, dsa->params.g, k, dsa->params.p, bctx); /* A r */
	BN_mod_mul(u, u, r, dsa->params.p, bctx);
	bighash(u, sdsa->params.p);
	BN_mod_exp(sdsa->params.q, dsa->priv_key, k, dsa->params.p, bctx); /* gbar */
	BN_mod_exp(sdsa->params.g, dsa->pub_key, k, dsa->params.p, bctx); /* ghat */
...
#endif
	/*3_0_13 update*/
	BN_mod_exp(u, sdsa->params.q, dsa->pub_key, dsa->params.p, bctx);
	BN_mod_exp(v, sdsa->params.g, dsa->priv_key, dsa->params.p, bctx);
	BN_mod_mul(u, u, v, dsa->params.p, bctx);
	BN_mod_inverse(u, u, dsa->params.p, bctx);
	BN_mod_mul(v, u, peer->iffval, dsa->params.p, bctx);
	...
	)
{
	 ...
	 */
	uptr = asn1cert;
	if ((cert = d2i_X509(NULL, (const unsigned char **)&uptr, len)) == NULL) {
	...
	)
{
	...
	ptr = (u_char *)ep->pkt;
	if ((req = d2i_X509(NULL, (const unsigned char **)&ptr, ntohl(ep->vallen))) == NULL) {
	...
	X509_NAME_add_entry_by_txt(subj, "commonName", MBSTRING_ASC,
	    (u_char *)sys_hostname, (int)strlen(sys_hostname), -1, 0);
	...
	)
{
	...
	cert = d2i_X509(NULL, (const unsigned char **)&ptr, ntohl(cinf->cert.vallen));
	...
	)
{
	 ...
	 */
	ret = cert_parse(data, (u_int)len, fstamp);
	efree(data);
	(void)fclose(str);
	if (ret == NULL)
		return (NULL);
	...
	)
{
	...
	if (dp != NULL)
		if(sscanf(++dp, "%u", &fstamp) != 1)
		{
		    msyslog(LOG_ERR, "crypto_tai: invalid filestamp %s\n", filename);
			fstamp = 0;
		}
...
crypto_setup(void)
{
	...
	if (rand_file == NULL) {
		snprintf(filename, MAXFILENAME, "%s/%s", keysdir, "rnd");
	 ...
	 */
	tmplen = i2d_PublicKey(host_pkey, NULL);
	if(tmplen <= 0)
	{
		msyslog(LOG_ERR, "crypto_setup: Construct public key failed");
		ipntp_exit_callback(ipcom_getpid());
	}
